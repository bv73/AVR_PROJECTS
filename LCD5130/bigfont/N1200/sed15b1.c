...
void sed15b1_putchar16(unsigned char ch, unsigned char x, unsigned char y)
{
  unsigned int addr;
...
{
  DDR_CSN=1; // Инициализация направлениия всех используемых ног
  DDR_SCL=1;
  DDR_SI=1;
  DDR_RESN=1;
  SCL_1;
...
void sed15b1_update_console(void)
{
  ...
  sed15b1_cmd(SED15B1_SETXADDR_LOWER); // X=0+add
