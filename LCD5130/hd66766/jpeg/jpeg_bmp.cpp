...
#define M_DQT   0xdb   
#define M_DRI   0xdd   
#define M_APP0  0xe0    //JFIF APP0 segment marker   
...
Beginning of file.*/   
    ...
    //get jpg file length   ÿ˜…û‰Í≈Ω÷çÀúŸﬂ⁄≈ﬂÊ‚å¬Ú€û≈‘√™∆ÿ   
    ...
    //ÁÏ≈‘Áø„”Å Õé‚·œå¡È‘ÍƒˆœÂÃÙ s ≈‘⁄Á n »≥ÁÊÕÍ≈‘Ê≈Ÿ¯ﬁåÊ≈ c®Å   
...
/*memcpy  
‰‡›Å extern void *memcpy(void *dest, void *src, unsigned int count);  
„”«£Å ÅÅi nclude  
…≤‘ÏÅ „Ÿsrc€öÊ»‘Íƒˆ◊º„ê»ƒÊ÷count»≥ÁÊÕÍ≈Õdest€öÊ»‘Íƒˆ◊º„ê®Å  
€≈”øÅ src ›dest€öÊ»‘Íƒˆ◊º„ê¬À‘ÏÊË≈¢Åç è⁄ß«≈ÀËÊ»ﬂêdest≈‘Ê»Â˚®Å*/   
...
{   
    ...
    while (!finish){   
        ...
        lp+=2;   
        switch (id){   
        case M_APP0:                         //JFIF APP0 segment marker   
            llength=MAKEWORD(*(lp+1),*lp);   //#define MAKEWORD(a, b) ((WORD)(((BYTE)(a)) | ((WORD)((BYTE)(b))) << 8))   
            lp+=llength;                    //œËÊ÷jpegÊ»Â˚Ê»ﬂêﬂ“‚À»≥ÁÊ∆ﬁ   
            break;   
        case M_DQT:                         //define quantization table   
            llength=MAKEWORD(*(lp+1),*lp);   
            qt_table_index=(*(lp+2))&0x0f;  //≈›4ﬁÀQT ’   
            lptemp=lp+3;   
            if(llength<80){                  //∆‰DQT∆ﬁÕº‡‡ƒ≤–˝why”À„‡Œé∆ÿ«Õ”ˆ≈‘ƒ≤–˝Åœƒ≤–˝⁄◊«∏„‡ƒ˝ÅœÅœÅœÅœÅœÅœÅœÅœÅœÅœÅœÅœÅœÅœ   
                for(i=0;i<64;i++)   
        ...
        case M_SOF0:                        //Start of frame 0   
            llength=MAKEWORD(*(lp+1),*lp);   
            ImgHeight=MAKEWORD(*(lp+4),*(lp+3));    //  ImgHeight   
            ImgWidth=MAKEWORD(*(lp+6),*(lp+5));     //ImgWidth   
            comp_num=*(lp+7);                       //components⁄ß—œ   
/*SOF0: Start Of Frame 0:  
  - $ff, $c0 (SOF0)  
  - √™∆ÿ (»ÔÁÊÕÍ, ≈›ÁÊÕÍ), 8+components*3  
  - ⁄ßŒÌŒé∆ÿ (1 byte) ”œ»≥·ö¡ŒﬁÀ⁄ß, ›£√Å⁄◊ 8 (ƒÉ∆⁄ßÿ˝Ã¢¬ÀÊØ√Ê 12  › 16)  
  - ›Ã÷ç»Ô∆ÿ (»ÔÁÊÕÍ, ≈›ÁÊÕÍ), ÿ˜…û¬ÀÊØ√Ê DNL Œ›¡Ë‡˚ >0  
  - ›Ã÷çœ˝∆ÿ (»ÔÁÊÕÍ, ≈›ÁÊÕÍ), ÿ˜…û¬ÀÊØ√Ê DNL Œ›¡Ë‡˚ >0  
  - components ⁄ß—œ(1 byte), À‚∆ÿ›Ã⁄◊ 1, YCbCr/YIQ ¬⁄Ÿé›Ã⁄◊ 3, CMYK ¬⁄Ÿé›Ã  
    ⁄◊ 4  
  - ”œ»≥ component: 3 bytes  
     - component id (1 = Y, 2 = Cb, 3 = Cr, 4 = I, 5 = Q)  
     - ¬Ÿ·öﬂ≈⁄ß (bit 0-3 vert., 4-7 hor.)  
     - quantization table  ’  
 */   
   
            if((comp_num!=1)&&(comp_num!=3))   
                return FUNC_FORMAT_ERROR;   
            if(comp_num==3){                    //·ÂŸé«Ê—œ⁄ß(number of color components)   
                comp_index[0]=*(lp+8);          //comp_index[3]„”–ƒƒÄƒˆ”œ»≥component id   
                SampRate_Y_H=(*(lp+9))>>4;    //¬Ÿ·öﬂ≈⁄ßÅ£bit0Å3vet,4-7horÅä   
     ...
     3)      for  (counter_y=1;counter_y<=VCr;counter_y++)  
                  for (counter_x=1;counter_x<=HCr;counter_x++)  
                     {  ∆‰ Cr ≈‘ Data Unit ¡“˚ }  
¿ƒﬁ‚Ÿﬂ”ˆ≈‘–ßÁ„: (HY=2, VY=2 ; HCb=VCb =1, HCr,VCr=1) Œ›⁄◊ÂÚ·ö‚À»≥ƒﬁ‡ê  
    YDU,YDU,YDU,YDU,CbDU,CrDU  
ÂÚ‡äŒ›”¯⁄≥—€‚Àœ˘ 16x16 ≈‘›Ã‡ﬁ. 16x16 = (Hmax*8 x Vmax*8) ÂÚ–ˇ Hmax=HY=2   
Vmax=VY=2‚À»≥ (Hmax*8,Vmax*8) ≈‘œ˘¡À√÷Áø MCU (Minimun Coded Unix) ◊¿”ˆ–ßÁ„Ê‡‚À»≥   
...
/*DHT: Define Huffman Table:  
 - $ff, $c4 (DHT)  
  - √™∆ÿ (»ÔÁÊÕÍ, ≈›ÁÊÕÍ)  
  - HT ‡’ﬂÄ (1 byte):  
     bit 0..3: HT  ’ (0..3, «∏‰êƒ˝ﬁÉ)  
     bit 4   : HT –‡›, 0 = DC table, 1 = AC table  
     bit 5..7: ¡Ë‡˚⁄◊ 0  
  - 16 bytes: √™∆ÿ⁄◊ 1..16 ƒú“˚≈‘«û ’⁄ß. ÂÚ 16 »≥⁄ß≈‘ ›„≤»” <=256  
  - n bytes: ‚À»≥¿ù ç—€¿ƒ≈Ì‰≥ƒﬁ‡êƒú“˚√™∆ÿ’’—‡≈‘«û ’¡˝(n = ƒú“˚ÁÏ⁄ß)  
 ¡»ÁÄ:  
  - ‚À»≥≈Ω∆–≈‘ DHT ∆ﬁœŸ‚‰¿ù ç∆»≥ HT, ”œ»≥∆Ã„‡Á‰Ã ≈‘‡’ﬂÄÁÊÕÍ*/   
        case M_DHT:                
            llength=MAKEWORD(*(lp+1),*lp);   
            if (llength<0xd0){   
                huftab1=(short)(*(lp+2))>>4;     //huftab1=0,1   
                huftab2=(short)(*(lp+2))&0x0f;   //huftab2=0,1   
                huftabindex=(huftab1<<1)+huftab2;   //huftabindex=huftab1*2+huftab2;   
                lptemp=lp+3;   
                for (i=0; i<16; i++)   
                ...
                while (hf_table_index!=0xff){   
                    ...
                    ccount=0;   
                    for (i=0; i<16; i++){   
                        code_len_table[huftabindex][i]=(short)(*(lptemp++));//code_len_table[huftabindex][i]¬À›çﬁÀ⁄ß“˚ÁÊ≈‘⁄ß—œ“˚√™¡˝   
                    ...
                    for (i=0; i<16; i++)   
                        if(code_len_table[huftabindex][i]!=0){   
                            ...
                            while(k<code_len_table[huftabindex][i])   
                            {   
                                code_value_table[huftabindex][k+j]=(short)(*(lptemp++));//code_value_table€ö„‡‚∆Á„ÕÍ≈Û€ö∆‰„≤≈‘ÿ£Ê≈   
                                k++;   
                            }   
                            j+=k;   
                        }   
                    ...
                    huf_max_value[huftabindex][i]=code_len_table[huftabindex][i]-1;   
                    for (j=i+1;j<16;j++){   
                        huf_min_value[huftabindex][j]=(huf_max_value[huftabindex][j-1]+1)<<1;     //huf_min_valueÁ≥⁄¬”ƒ„”Åœ   
                        huf_max_value[huftabindex][j]=huf_min_value[huftabindex][j]+code_len_table[huftabindex][j]-1;   
                    }   
                    code_pos_table[huftabindex][0]=0;   
                    for (j=1;j<16;j++)   
...
/*DRI: Define Restart Interval:  
  - $ff, $dd (DRI)  
  - √™∆ÿ (»ÔÁÊÕÍ, ≈›ÁÊÕÍ), ¡Ë‡˚⁄◊ 4  
  - MCU œ˘≈‘≈Ω‰åÊ‡≈‘ÊË‡“œå⁄ÃÃÙ»∫ (»ÔÁÊÕÍ, ≈›ÁÊÕÍ),  
    ‚Ú€Ã⁄◊€≈, ”œ n »≥ MCU œ˘Œ›„‡‚À»≥ RSTn ¡˙Ã◊.  
    ≈Í‚À»≥¡˙Ã◊⁄◊ RST0, ÿÀ É⁄◊ RST1 ≈ÿ, RST7  É‰Èƒ„ RST0 ÊË»ƒ */   
        case M_DRI:                
            llength=MAKEWORD(*(lp+1),*lp);   
...
/*SOS: Start Of Scan:  
- $ff, $da (SOS)  
  - √™∆ÿ (»ÔÁÊÕÍ, ≈›ÁÊÕÍ), ¡Ë‡˚⁄◊ 6+2*(Ÿ£”¯‡‡‘ÍÁ˘Ã¢≈‘⁄ß—œ)  
  - Ÿ£”¯‡‡‘ÍÁ˘Ã¢≈‘⁄ß—œ (1 byte), ¡Ë‡˚ >= 1 , <=4 («∏‰ê⁄◊ƒ˝≈‘) ›£√Å⁄◊ 3  
  - ”œ»≥Á˘Ã¢: 2 bytes  
     - component id (1 = Y, 2 = Cb, 3 = Cr, 4 = I, 5 = Q), Ãû SOF0  
     - ⁄…„”≈‘ Huffman ¡˝:  
        ...
        case M_SOS:   
            ...
            for (i=0;i<comp_num;i++){   
                if(*lptemp==comp_index[0]){   
                    ...
                    YAcIndex=((*(lptemp+1))&0x0f)+2;    //ﬁå⁄¬”ƒÀÒÃ„2ÅœÅœÅœÅœÅœÅœÅœ‘·≈–⁄◊ÊÀ€≈”ø⁄◊0À…⁄◊1 ’¡˝   
                }   
                else{   
...
//›Ã÷çÕÒ⁄º   
    ...
    return FUNC_OK;             //#define FUNC_OK 0   
}   
/////////////////////////////////////////////////////////////////   
void InitTable()   
{   
...
{   
    ...
    Y_in_MCU=SampRate_Y_H*SampRate_Y_V;  //SampRate_Y_H€°÷Õ¬Ÿ·öﬂ≈⁄ßSampRate_Y_Vƒ…Ê¡¬Ÿ·öﬂ≈⁄ß   
    ...
    while((funcret=DecodeMCUBlock())==FUNC_OK){   
        ...
        if((restart)&&(interval % restart==0))   
             IntervalFlag=TRUE;          //DRI ÊË‡“œå⁄Ã¡˙ÊŒIntervalFlagÿéŒÊ¡Ù—œ   
        else   
            IntervalFlag=FALSE;   
...
{   
    ...
    if (IntervalFlag){          //»öŒÌIntervalFlag–ƒ’‡∆ﬂ⁄◊«∏œå⁄ÃÊË‡“ÕÚ“˚   
        lp+=2;   
        ycoef=ucoef=vcoef=0;   
        BitPos=0;   
        CurByte=0;   
    }   
    switch(comp_num){    //comp_num¡˝⁄ŒÁ˘Ã¢⁄ß   
    case 3:   
        ...
        {   
            ...
            if (funcret!=FUNC_OK)   
                return funcret;   
            BlockBuffer[0]=BlockBuffer[0]+ycoef;    //ycoefﬁåÊ¡—ø«Ê—œ¬˛Ê≈   
            ycoef=BlockBuffer[0];                   //Áøﬁåﬂ“‚À»≥Ê¡—ø«Ê—œ≈‘ÀöÊ≈   
            for (j=0;j<64;j++)   
                *lpMCUBuffer++=BlockBuffer[j];  //short BlockBuffer[64];BlockBufferÁøﬁåÕÚ“˚ É≈‘⁄ßŒÌ≈‘À ƒˆ   
        }   
...
{   
    ...
    funcret=DecodeElement();      //ﬂÿ∆‰Ê¡—øÕº‡‡ÕÚ“˚   
    if(funcret!=FUNC_OK)   
        return funcret;   
    ...
    while (count<64){   
        ...
        if(funcret!=FUNC_OK)  //#define FUNC_OK 0   
            return funcret;   
        if ((rrun==0)&&(vvalue==0)){   
            for (i=count;i<64;i++)   
                BlockBuffer[i]=0;   
...
/////////////////////////////////////////////////////////////////////////////   
//DecodeElement()∆‰Ê¡—ø®ÄÕÀ—ø⁄ßŒÌÕº‡‡ÕÚ“˚   
int DecodeElement()   
{   
...
//BYTE And[9]={0,1,3,7,0xf,0x1f,0x3f,0x7f,0xff};    
    if(BitPos>=1){   
        ...
        CurByte=CurByte&And[BitPos];  //CurByteﬁåÿéŒÊ¡Ù—œÅçƒˆƒÄLPÊ»Â˚ÿ®≈”≈‘⁄ßŒÌ   
    }   
    else{   
        lastbyte=ReadByte();   
        BitPos--;                       //short BitPos   
    ...
    hufexbyte=(BYTE)code_value_table[HufTabIndex][temp];  //temp⁄◊Ã◊“Ã¡“˚code_value_table[HufTabIndex][temp]≈‘ﬁÀÊ”   
    rrun=(short)(hufexbyte>>4);                           //≈”≈Õ◊¿—Û≈‘»≥⁄ß   
    runsize=hufexbyte&0x0f;                             //(◊¿—Û≈‘»≥⁄ßÅç“˚√™Ê≈)≈”≈Õ“˚√™Ê≈®Å   
    if(runsize==0){   
        vvalue=0;   
        return FUNC_OK;   
    }   
    ...
    if(BitPos>=runsize){   
        BitPos-=runsize;   
        valueex=(BYTE)CurByte>>BitPos;                     //≈”≈Õ“˚Ê≈   
        CurByte=CurByte&And[BitPos];   
    }   
    else{   
        valueex=CurByte;   
        ...
        lastbyte=ReadByte();   
        BitPos-=tempsize;   
        valueex=(valueex<<tempsize)+(lastbyte>>BitPos);     //valueex¡Åƒˆ“˚Ê≈   
        CurByte=lastbyte&And[BitPos];                       //CurByte¡Åƒˆƒßƒ≤–˝⁄ßŒÌ   
    }  //else   
    sign=valueex>>(runsize-1);   
    if(sign)                                                //ÕÚ√≥ﬂ≈⁄ßÊ≈Åçvvalue¡Åƒˆ÷Ùﬂ≈⁄ßÊ≈   
        vvalue=valueex;   
    else{   
...
{   
    ...
    for(i=0;i<8;i++)   
        for(j=0;j<8;j++){   
            tag=Zig_Zag[i][j];   
            buffer2[i][j]=(int)s[tag]*(int)pQt[tag];        //«ƒZig-zag¡“˚ÕÒ⁄º   
        }   
    buffer1=(int *)buffer2;   
    Fast_IDCT(buffer1);                                     //«ƒﬂê–˚ŸÄ„‡ß¡ÙÀÀ   
    for(i=0;i<8;i++)   
        for(j=0;j<8;j++)   
            d[i*8+j]=buffer2[i][j]+offset;                  //‹Ò◊¿Ã„128   
}   
///////////////////////////////////////////////////////////////////////////////   
void Fast_IDCT(int * block)   
{   
