...
unsigned char pcf8814_read (void)
{
  SDIN_0; // Pullup off DAT pin
  DDR_SDIN=0; // Pin DAT for Input
...
//  SCLK_0;
  ...
  {
    delay_us(10);
    SCLK_1; // Чтение по нарастанию SCLK, данные выставились
    delay_us(10);
    if (PIN_PORT&(1<<PB7)) // Берем значение бита
      {
...
unsigned char pcf8814_read_id(void)
{
  pcf8814_cmd(PCF8814_READ_ID);
  unsigned char id=pcf8814_read();
//  pcf8814_cmd(PCF8814_SOFTWARE_RESET);
  pcf8814_init();
  return (id);
}
