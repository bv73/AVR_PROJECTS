      ...
      for(i=0; i<7; i++)            // Shows all the colors in 2 strip
        {
          ...
          nokia_outs(CON,1,2+(6*8));          // next line ( each line=8 bits )
          nokia_outs(CON,1,2+(7*8));          // next line ( each line=8 bits )
          delay_ms(1500);
        }
   }
