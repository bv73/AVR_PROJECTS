...
// ********************************************************************
// Written by (R)soft 17.10.2011 Last Edition: 17 Oct 2011
// Version 1.0
// Tested on Mega2561 & Mega162.
// Testing on Alcatel OT 311 LCD Module. Resolution 96x65 pixels.
// Compatible with Alcatel 511 and 525 LCD Modules.
// Built-in SED15B1 compatible controller with SPI interface.
...
void sed15b1_data(unsigned char dat)
{
  ...
  sed15b1_write(dat);
//  if (++sed_x > (SED15B1_PIXELX-1)) // Модифицируем позицию в экранном буфере
//  {
//    // Если был достигнут конец экрана то следующая строка
//    sed_x=0;
//    sed_y++;
//  }
}
...
{
  unsigned int addr = char_to_addr(ch); // Вычисление адреса начала символа (общая процедура)
  // Вывод пяти байт
  for (ch=0; ch<5; ch++)  
    {  
