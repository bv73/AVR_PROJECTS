...
void InitUART (void)
{
  ...
  if (PINE&(1<<ADDR)) // Смотрим состояние ноги адреса
    {
      Net_Addr=1;  // Если единица, то адрес в сети @1
    }
  else
    {
      Net_Addr=2;  // Если ноль, то адрес в сети @2
    }
}
...
/* Переполнение буфера далее в коде ограничиваем величиной байта 0xFF, это лучше чем лишняя проверка
  Этот кусок отключен.
  if (HowMany_RX > SIZE_BUF-1)  // Если счетчик принятых байт больше чем длина буфера, то буфер переполнился
    {
      TimeOut_RX=0; // Установка счетчика таймаута в ноль
      HowMany_RX=0;  // Сброс счетчика принятых байт и вываливаемся из прерывания.
      return;
...
/*  
  if (HowMany_RX==1)  // Если это второй байт, то проверяем номер функции
     {
       switch (UART_Buf[1])
         {
           // Проверка на валидные номера функций
           case 3: break;  // Если второй байт=3 то ОК
           case 6: break;  
           case 8: break;
           case 16: break;
           default:  // Все остальные номера функции - ошибка
                 {
                   TimeOut_RX=0; // Установка счетчика таймаута в ноль
                   HowMany_RX=0;  // Сброс счетчика принятых байт и вываливаемся из прерывания.
                   return;
...
__interrupt void USART_TX(void)
{
  if (HowMany_TX >= Pointer_TX)
  ...
  else
    {
      ...
      HowMany_RX=0;
      HowMany_TX=0;
      LED_PORT|=(1<<LED_TX);  // Гасим LED_TX
    }
}
...
//    {
      LED_PORT&=~(1<<LED_TX); // Включаем LED - начало передачи
