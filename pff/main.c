...
//  init_devices();
  ...
  sprintf(LN1,"Disk Init...");
//  CardType=0;
//  error=disk_initialize();
  sprintf(LN2,"Error=%02x",error);
/*
  if(error)
  {
    if(error == 1) sprintf(LN3,"Card not detect");
    if(error == 2) sprintf(LN3,"Card Init failed");
    pcf8814_update_console();    
    while(1);
  }
  ...
  switch (CardType)
  {
    case 1: sprintf(LN3,"Standard Capacity Card (Ver 1.x) Detected!");
  		 break;
    case 2: sprintf(LN3,"High Capacity Card Detected!");
  		 break;
    case 3: sprintf(LN3,"Standard Capacity Card (Ver 2.x) Detected!");
  		 break;
    default: sprintf(LN3,"Unknown SD Card Detected!");
  		 break; 
    }
  pcf8814_update_console();
*/
...
/*
 N93_putpixel16(10,10,BLACK);
  N93_draw(200,10,20,300,GREEN_COLOR);
  N93_setrect(10,50,100,150,1,GREEN_COLOR);
  N93_setrect(10,50,100,150,0,WHITE_COLOR);
