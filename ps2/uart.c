...
void InitUART (void)
{
  ...
  // RXEN0=1  По умолчанию приемник выключен
  // TXEN0=0  По умолчанию передатчик включен
  // RXCIE0=1 Receive Interrupt Enable
  // TXCIE0=1 Transmit Interrupt Enable

  UCSR0B = (0<<RXEN0)|(1<<TXEN0)|(1<<RXCIE0)|(1<<TXCIE0);
...
__interrupt void USART_RX(void)
{
  ...
  TimeOut_RX=TIMEOUT_UART; // Переустановка счетчика таймаута по приему (~3 ms)
